package com.hotel.booking.user_services.paystack.serviceImpl;
...
import com.hotel.booking.user_services.paystack.dto.PaystackPaymentRequest;
import com.hotel.booking.user_services.paystack.dto.PaystackPaymentResponse;
import com.hotel.booking.user_services.paystack.dto.PaystackPaymentVerificationResponse;
import com.hotel.booking.user_services.paystack.dto.PaystackTransactionFetchResponse;
import com.hotel.booking.user_services.paystack.dto.PaystackTransactionListResponse;
...
import com.hotel.booking.user_services.paystack.enums.PaystackPaymentStatus;
import com.hotel.booking.user_services.paystack.repository.TransactionRepository;
import com.hotel.booking.user_services.paystack.repository.TransactionVerificationRepository;
...
public class PaystackServiceImpl implements PaystackService {
    ...
    public ResponseModel verifyTransaction(String reference) {
        ...
        /* if (response.getStatusCode().is2xxSuccessful()) {
            PaystackPaymentVerificationResponse paystackPaymentVerificationResponse = response.getBody();

            if (paystackPaymentVerificationResponse != null && paystackPaymentVerificationResponse.isStatus()
                    && "SUCCESS".equalsIgnoreCase(paystackPaymentVerificationResponse.getData().getStatus())) {

                Optional<TransactionVerification> existingVerification = transactionVerificationRepository.findByTransactionReference(reference);
                if (existingVerification.isPresent()) {
                    return new ResponseModel(HttpStatus.OK.value(), "Transaction already verified", paystackPaymentVerificationResponse);
                }
                TransactionVerification verification = new TransactionVerification();
                    verification.setTransactionId(paystackPaymentVerificationResponse.getData().getId());
                    verification.setFirstName(paystackPaymentVerificationResponse.getData().getCustomer().getFirst_name());
                    verification.setLastName(paystackPaymentVerificationResponse.getData().getCustomer().getLast_name());
                    verification.setEmail(paystackPaymentVerificationResponse.getData().getCustomer().getEmail());
                    verification.setPhoneNumber(paystackPaymentVerificationResponse.getData().getCustomer().getPhone());
                    verification.setVerified(true);
                    verification.setStatus(paystackPaymentVerificationResponse.getData().getStatus());
                    verification.setGatewayResponse(paystackPaymentVerificationResponse.getData().getGateway_response());
                    verification.setAmount(paystackPaymentVerificationResponse.getData().getAmount() / 100);//Converting back from kobo to naira
                    verification.setPaidAt(paystackPaymentVerificationResponse.getData().getPaid_at());
                    verification.setChannel(paystackPaymentVerificationResponse.getData().getChannel());
                    verification.setCurrency(paystackPaymentVerificationResponse.getData().getCurrency());
                    verification.setIpAddress(paystackPaymentVerificationResponse.getData().getIp_address());
                    verification.setCreatedAt(LocalDateTime.now());
                    transactionVerificationRepository.save(verification);

                    Optional<Transaction> transaction = transactionRepository.findByReference(reference);
                    if (transaction.isPresent()) {
                        Optional<User> user = userRepository.findByEmail(transaction.get().getEmail());
                        Booking booking = bookingRepository.findByBookingCode(transaction.get().getBookingCode()).orElseThrow(
                            () -> new GlobalRequestException(String.format(Message.NOT_FOUND, "Booking"), HttpStatus.NOT_FOUND)
                        );
                        if(user.isPresent()){
                            if(booking.getUser().getUserCode().equals(user.get().getUserCode())){
                                transaction.get().setPaystackPaymentStatus(PaystackPaymentStatus.SUCCESS);
                                transactionRepository.save(transaction.get());
                                booking.setPaymentStatus(PaymentStatus.PAID);
                                bookingRepository.save(booking);
                            }else{
                                transaction.get().setPaystackPaymentStatus(PaystackPaymentStatus.FAILED);
                                transactionRepository.save(transaction.get());
                                booking.setPaymentStatus(PaymentStatus.UNPAID);
                                bookingRepository.save(booking);
                            }
                    ...
                    return new ResponseModel(HttpStatus.OK.value(), "Transaction verified successfully", paystackPaymentVerificationResponse);
            } else {
                Optional<Transaction> transaction = transactionRepository.findByReference(reference);
                if (transaction.isPresent()) {
                    Optional<User> user = userRepository.findByEmail(transaction.get().getEmail());
                ...
                return new ResponseModel(HttpStatus.BAD_REQUEST.value(), "Transaction verification failed", paystackPaymentVerificationResponse);
            }
        }else {
            return new ResponseModel(HttpStatus.BAD_REQUEST.value(), "Transaction verification failed", null);
        } */
